...
description: "calibrate_content aligns buyers and sellers on brand suitability standards through structured dialogue before AdCP campaign execution begins."
"og:title": "AdCP — calibrate_content"
---
...
**Schema**: [calibrate-content-request.json](https://adcontextprotocol.org/schemas/3.2.0-rc.4/content-standards/calibrate-content-request.json)
...
**Schema**: [calibrate-content-response.json](https://adcontextprotocol.org/schemas/3.2.0-rc.4/content-standards/calibrate-content-response.json)

### Passing Response

```json
{
  "$schema": "/schemas/3.2.0-rc.4/content-standards/calibrate-content-response.json",
  "status": "completed",
  "verdict": "pass",
...
Calibration supports a sequence of typed evaluations using A2A conversation
correlation. Each turn remains a complete `calibrate_content` invocation; a
`contextId` groups the evaluations but does not make free text authoritative.
...
const response1 = await mcp.call('calibrate_content', {
  standards_id: "example_brand_safety",
  artifact: {
    property_rid: "01916f3a-a1d3-7000-8000-000000000040",
    artifact_id: "r_news_politics_123",
    assets: [
...
// A later call on the same MCP transport session does not send context_id.
...
// Try a different artifact on the same transport session.
