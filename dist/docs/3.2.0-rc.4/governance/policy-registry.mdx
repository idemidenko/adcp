...
Governance agents are LLMs that interpret natural language policy text -- the same pattern used by [Content Standards](/dist/docs/3.2.0-rc.4/governance/content-standards/index). The registry's value is in structured metadata and calibration exemplars, not a custom rule language.
...
Each policy in the registry follows the [policy-entry schema](https://adcontextprotocol.org/schemas/3.2.0-rc.4/governance/policy-entry.json):
...
For the operational pattern — how to pin a policy version, what happens when a registry policy version-bumps mid-campaign, how to use `effective_date` for staged adoption, the additive-only rule for inline `custom_policies`, and a working-group FAQ — see [Sync and versioning](/dist/docs/3.2.0-rc.4/governance/policy-registry-sync).
...
Brands reference registry policies through their compliance configuration. See the [Campaign Governance specification](/dist/docs/3.2.0-rc.4/governance/campaign/specification#brand-compliance-configuration) for the conceptual model.
...
|--------|------------------------------|
| **[Campaign Governance](/dist/docs/3.2.0-rc.4/governance/campaign/index)** | Resolves policies via brand compliance config, evaluates actions against policy text in `check_governance` |
| **[Content Standards](/dist/docs/3.2.0-rc.4/governance/content-standards/index)** | Creates content standards from registry policies using `registry_policy_ids` |
| **[Property Governance](/dist/docs/3.2.0-rc.4/governance/property/index)** | Declares `registry:` prefixed features in `get_adcp_capabilities`, evaluates properties against policy text |
| **[Creative Governance](/dist/docs/3.2.0-rc.4/governance/creative/index)** | Declares `registry:` prefixed creative features, evaluates creatives for AI disclosure and content compliance |
| **[Media Buy](/dist/docs/3.2.0-rc.4/media-buy/index)** | Sellers declare `enforced_policies` on products, buyers send `required_policies` in requests |
...
<Note>
The `registry:` convention is the **delegated** form of policy reference — buyer asks the seller's governance agent to evaluate the policy as a binary feature. The complementary **attribution** pattern, where producers tag mechanism-level filters and measurements with `policy_id` to record buyer-chosen thresholds and motivated evaluations, lives at [Policy Attribution](/dist/docs/3.2.0-rc.4/governance/policy-attribution).
</Note>

The same registry entry can provide an actionable creative rejection vocabulary without creating a protocol enum. Governance agents advertise `registry:{policy_id}` compliance features, while sellers cite the bare `policy_id` in existing `CREATIVE_REJECTED` details. The registry owns the shared rule and calibration examples; the agent owns detection methodology; the seller owns the enforcement decision. See [Policy-backed creative rejections](/dist/docs/3.2.0-rc.4/governance/creative/policy-backed-rejections).
...
See [Annex III & Art 22 obligations](/dist/docs/3.2.0-rc.4/governance/annex-iii-obligations) for deployer guidance and the distinction between `reallocation_threshold` (budget reallocation) and `human_review_required` (decisions affecting individuals).
...
Data providers can reference these definitions when declaring `restricted_attributes` on their signal definitions. See [Declaring governance metadata](/dist/docs/3.2.0-rc.4/signals/data-providers#declaring-governance-metadata). Governance agents match signal-declared attributes against plan-level `restricted_attributes` during [`check_governance`](/dist/docs/3.2.0-rc.4/governance/campaign/tasks/check_governance) validation — signals with matching restricted attributes are blocked from targeting for that campaign.
