...
description: "Property list tasks in AdCP create, update, get, list, and delete inclusion and exclusion lists combining static sets with dynamic filters."
"og:title": "AdCP — Property List Management"
---
...
|------|---------|---------------|
| [`create_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#create_property_list) | Create a new property list | ~500ms |
| [`update_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#update_property_list) | Modify an existing list | ~500ms |
| [`get_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#get_property_list) | Retrieve list with resolved properties | ~2-5s (depending on size) |
| [`list_property_lists`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#list_property_lists) | List all property lists | ~500ms |
| [`delete_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#delete_property_list) | Delete a property list | ~200ms |
...
The brand reference uses the standard [core/brand-ref](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/brand-ref.json) schema. The governance agent resolves the domain to discover the brand's identity via its `brand.json` file.

## Webhooks

Configure webhooks via [`update_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#update_property_list) to receive notifications when the resolved list changes.

**Important**: Webhooks provide **notification only**. They tell you that the list has changed, but do not stream the updated properties. After receiving a webhook, you must call [`get_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#get_property_list) to retrieve the updated property set.
...
Recipients MUST verify the delivery's RFC 9421 `Signature`, `Signature-Input`, and `Content-Digest` HTTP headers before processing, following the [webhook profile](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security#webhook-callbacks). The required body-level `signature: "rfc9421"` is a deprecated compatibility marker retained through 3.x; receivers MUST ignore it for authentication. Recipients MUST also dedupe by `idempotency_key` so retried deliveries of the same change event are ignored.
...
Filters are applied when the list is resolved (via [`get_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#get_property_list)):
...
Feature requirements reference features discovered via [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities). Each agent exposes different features (mfa_score, carbon_score, coppa_certified, etc.).
...
See the [base-property-source schema](https://adcontextprotocol.org/schemas/3.2.0-rc.4/property/base-property-source.json) for the full specification.
...
<Note>
The `auth_token` is only returned when the list is created via [`create_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#create_property_list). Store it securely - you'll need it to share access with sellers.
</Note>
...
| Code | Description |
|------|-------------|
| [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-reference-not-found) | Property list ID doesn't exist, or the caller lacks access. Returned uniformly for both cases — see [Uniform response for inaccessible references](/dist/docs/3.2.0-rc.4/building/by-layer/L3/error-handling#standard-error-codes). Sellers MUST NOT distinguish "exists but unauthorized" from "does not exist." |
| [`VALIDATION_ERROR`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-validation-error) | Filter configuration is invalid, or a list with the requested name already exists. Correct the filter values or choose a unique name. |

## Caching and Refresh

The [`get_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#get_property_list) response includes caching guidance. When it supplies `resolved_at`, sellers copy that timestamp into product `list_applications` receipts; otherwise the seller timestamps completion of its full snapshot fetch, including all pages:
