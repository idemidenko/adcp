...
description: "Task reference for AdCP property governance — property list management, feature evaluation, and delivery validation tasks."
"og:title": "AdCP — Property Governance Tasks"
...
|------|---------|---------------|
| [get_adcp_capabilities](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) | Discover agent capabilities | ~200ms |

Use the protocol-level [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) task to discover what features a governance agent can evaluate. See the [governance section](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities#governance-protocol) for details on the `property_features` array.
...
Use [`create_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#create_property_list) with filters and brand reference:
...
Filters have two built-in fields (`countries_all`, `channels_any`) plus `feature_requirements` which reference features the agent provides (discovered via [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities)). For quantitative features, use `min_value`/`max_value`. For binary or categorical features, use `allowed_values`.

### Getting Resolved Properties

Use [`get_property_list`](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists#get_property_list) to retrieve the list with resolved identifiers:
