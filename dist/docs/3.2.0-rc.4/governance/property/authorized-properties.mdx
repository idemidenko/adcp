...
description: "Authorized properties in AdCP let buyers restrict campaigns to verified inventory using property lists and supply path validation."
"og:title": "AdCP — Understanding Authorization"
---
...
3. **Agent Discovery**: Sales agents declare their portfolio via [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) in the `media_buy.portfolio` section

The first two create bilateral verification — like `ads.txt` + `sellers.json` in programmatic. For delegated or network paths, both sides must agree for the supply path to be trusted. See [ad networks](/dist/docs/3.2.0-rc.4/sponsored-intelligence/networks) for the full pattern.
...
- **authorized_agents**: List of sales agents authorized to represent properties
  ...
  - **delegation_type**: Whether this path is `direct`, `delegated`, or `ad_network`
  - **collections**: Optional collection selectors that narrow authorization to specific content programs. The selector's `publisher_domain` may be an external channel owner's domain (owner-sold carriage), and a selector without `collection_ids` is a bulk grant for every collection declared at that domain — see [owner-sold channel carriage](/dist/docs/3.2.0-rc.4/governance/property/adagents#owner-sold-channel-carriage)
  - **placement_ids**: Optional placement IDs from the publisher's placement registry in `adagents.json`
...
Sales agents use the [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) task to declare their portfolio information in the `media_buy.portfolio` section. This serves multiple purposes:
...
const result = await salesAgent.call('get_products', {
  idempotency_key: "550e8400-e29b-41d4-a716-446655442070",
  buying_mode: "brief",
  brief: "Chicago radio ads",
});
...
- **[`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities)** - Discover agent capabilities and portfolio information
- **[Product Discovery](../../media-buy/product-discovery/)** - How authorization integrates with product discovery
- **[Properties Schema](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/property.json)** - Technical property data model
