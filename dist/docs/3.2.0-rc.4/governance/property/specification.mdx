...
description: "Formal specification for AdCP property governance — property models, feature evaluation, list management, and delivery validation."
"og:title": "AdCP — Property Governance Specification"
...
- `REFERENCE_NOT_FOUND`: Referenced property, policy, or property list does not exist or is not accessible by the caller. Returned uniformly for both "does not exist" and "exists but unauthorized" — see [Uniform response for inaccessible references](/dist/docs/3.2.0-rc.4/building/by-layer/L3/error-handling#standard-error-codes).
...
See the [adagents.json Tech Spec](/dist/docs/3.2.0-rc.4/governance/property/adagents#governance-agent-discovery) for the complete discovery workflow.
...
- See the [adagents.json Tech Spec](/dist/docs/3.2.0-rc.4/governance/property/adagents) for property declaration and authorization
- See the [get_adcp_capabilities task reference](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) for capability discovery
- See the [Property List Management](/dist/docs/3.2.0-rc.4/governance/property/tasks/property_lists) for CRUD operations and webhooks
- See the [validate_property_delivery task reference](/dist/docs/3.2.0-rc.4/governance/property/tasks/validate_property_delivery) for post-campaign compliance validation
