...
description: "get_account_financials returns spend summaries, credit balances, payment status, and invoice history for operator-billed AdCP accounts. Requires account_financials capability."
"og:title": "AdCP — get_account_financials"
testable: false
---
...
`get_account_financials` is only available when the seller declares `account_financials: true` in [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities). It applies to **operator-billed accounts** only. For agent-billed accounts, the agent's own billing system is the source of truth.

**Response Time**: ~1s.

**Request Schema**: [`/schemas/3.2.0-rc.4/account/get-account-financials-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/account/get-account-financials-request.json)
**Response Schema**: [`/schemas/3.2.0-rc.4/account/get-account-financials-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/account/get-account-financials-response.json)
...
|-----------|------|----------|-------------|
| `account` | object | Yes | [Account reference](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents#account-references) — by `account_id` or natural key (`brand` + `operator` + optional `sandbox`). Must be an operator-billed account. |
...
| `timezone` | IANA timezone of the seller's billing day boundaries (e.g., `America/New_York`). All dates in the response are calendar dates in this timezone. It may differ from `Account.timezone`, reporting timezone, or a feature-specific cap timezone; use this explicit value for financial periods. |
...
|------------|-------------|------------|
| [`UNSUPPORTED_FEATURE`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-unsupported-feature) | Account uses agent billing — financials not available from seller | Query your own billing system for agent-billed accounts |
...
| [`ACCOUNT_NOT_FOUND`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-account-not-found) | Account does not exist or is not accessible | Check account reference or re-sync |

## Next steps

- [list_accounts](/dist/docs/3.2.0-rc.4/accounts/tasks/list_accounts) -- Discover accounts and check their status
- [get_adcp_capabilities](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) -- Check `account_financials` before calling this task
- [Accounts and agents](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents) -- Billing models and account references
