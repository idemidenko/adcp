...
Authoritative version-by-version release record for AdCP, with cumulative change detail, migration guidance, and per-version adoption notes. For the 3.2 adopter overview, see [What's New in AdCP 3.2](/dist/docs/3.2.0-rc.4/reference/whats-new-in-3-2); for the upgrade checklist, see [Migrating from 3.1 to 3.2](/dist/docs/3.2.0-rc.4/reference/migration/3-1-to-3-2); and for prerelease artifact and SDK timing, see the [3.2 prerelease program](/dist/docs/3.2.0-rc.4/reference/3-2-beta). For the at-a-glance status of every published version, see [Versions & Compatibility](/dist/docs/3.2.0-rc.4/reference/versions). For source-level changelogs, see [CHANGELOG.md](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md). For version stability, schema-change scope, and the 3.x guarantees see [Versioning & Governance](/dist/docs/3.2.0-rc.4/reference/versioning). For v2 end-of-life see the [v2 sunset page](/dist/docs/3.2.0-rc.4/reference/v2-sunset).

---

## Version 3.2.0

**Status:** Release-candidate cycle — minor release targeting the 3.2.0 milestone. RC.3 is the current signed protocol checkpoint. It hardens the experimental Reliable Reporting consumer-status loop, adds a shared MediaBuy-level frequency cap with structured product frequency-cap constraints, and adds request-only targeting inputs with `null`-clear semantics. TypeScript `@adcp/sdk@14.0.0-rc.40` embeds RC.3 exactly; Python `adcp==8.0.0b14` and Go `adcp/v3@v3.2.1` still embed RC.1, with role-dependent higher-level coverage. Stable wire shapes remain backward compatible except for the ratified `media_buy_status` cleanup and request-signature encoding migration described below; explicitly experimental surfaces may carry noticed changes under the [experimental-status contract](/dist/docs/3.2.0-rc.4/reference/experimental-status).

<Warning>
**Reliable Reporting breaking-change notice — published September 12, 2026.**
RC.2 publishes opt-in `sync_reporting_status` so a buyer can report missing,
unreadable, contradictory, or exactly received reporting back to the seller; the
seller can then detect when a received revision becomes stale after restatement.
RC.3 hardens the loop: a stale `received` gets a bounded re-read grace
window before it escalates, buyers owe a status by
`expected_at + automated_recovery_window_seconds` (counted in
`obligation_counts.consumer_status_pending`, never in health), issues carry
`opened_at` / `issue_state` / `external_ref` with an optional
`consumer_mismatch_escalation_seconds` and `operations_contact`, and
`content_mismatch` names contract-fact disagreements through a closed
`mismatch_code`. RC.3's `reporting-delivery-config.json` also reserves
`authoritative_party`; sellers MUST reject `consumer` with `UNSUPPORTED_FEATURE`
until a later minor defines the buyer-deposited revision task
([#7440](https://github.com/adcontextprotocol/adcp/issues/7440)). Sellers advertise
`consumer_status_task` only when implemented; RC.1 remains unchanged. The next
eligible minor makes this task required Core no earlier than October 24, 2026.
See the [migration guide](/dist/docs/3.2.0-rc.4/reference/migration/reliable-reporting-status-loop).
</Warning>
...
| `3.2.0-rc.1` | Complete the experimental Reliable Reporting 1.0 contract and harden delivery currency, aggregate reach, vendor-metric identity, and integration behavior | Previous checkpoint with exact Python support in `adcp==8.0.0b14` and exact Go support in `adcp/v3@v3.2.1` |
| `3.2.0-rc.2` | Add the opt-in experimental Reliable Reporting `sync_reporting_status` consumer-status loop, buyer and orchestrator storyboards with a buyer-orchestrator compliance track, cursor pagination for truncated `get_media_buy_delivery` breakdowns, sub-country product coverage filters, and restored `canonical-forecast-point` constraints | Previous checkpoint. No maintained SDK embeds it; test it from the signed bundle |
| `3.2.0-rc.3` | Harden the consumer-status loop (`content_mismatch` and `mismatch_code`, a bounded grace window for a stale `received`, the buyer posting deadline and `obligation_counts.consumer_status_pending`, issue lifecycle fields, `operations_contact`, and the reserved `authoritative_party`), add a shared MediaBuy-level `frequency_cap` with structured product frequency-cap constraints, add request-only targeting with `null`-clear semantics, and retire superseded beta artifacts | Current signed protocol checkpoint with exact TypeScript support in `@adcp/sdk@14.0.0-rc.40`; Python and Go remain on RC.1 |

Each prerelease is immutable. The `3.2-rc` documentation selector advances to the latest release candidate; the `3.2-beta` selector remains frozen at beta.11. Pinned protocol, schema, compliance, and documentation artifacts remain available. See the [3.2 prerelease program](/dist/docs/3.2.0-rc.4/reference/3-2-beta).
...
- **Targeting and audiences:** targeting-aware discovery, portable demographic predicates, named places, value-aware regions, browser families, property exclusions, audience evidence, exact package readback, and request-only targeting inputs where each dimension distinguishes omission, a non-null replacement, and an explicit `null` that clears it.
- **Frequency management:** a shared MediaBuy-level `frequency_cap` counted once across every package, advertised through `media_buy.aggregate_frequency_capping` and product `media_buy_support`, plus structured product frequency-cap constraints on `overlay_support.frequency_cap_support` and the `update_media_buy_frequency_cap` action.
...
- **Errors and recovery:** required `error.recovery` emission for 3.2 producers with schema-compatible 3.1 decoding, bounded unknown-code fallback, and reference vectors for classification and `retry_after` scheduling.
...
- **Reliable Reporting:** a separately discovered, versioned experimental three-tier contract for expected-period obligations, immutable official revisions, managed materializations, authenticated receipts, correction ledgers, and repairable status. RC.1 completes Reliable Reporting 1.0; RC.2 adds the opt-in two-sided `sync_reporting_status` consumer-status loop; RC.3 hardens that loop and reserves `authoritative_party` for a later minor.
...
|---|---|
| A buyer | Discover the exact prerelease, gate every compact tool and optional field on capabilities, keep a 3.1 route during prerelease, and retain bounded recovery fallback for legacy errors. |
| A seller or service | Advertise and echo the exact prerelease served, populate `error.recovery` on every 3.2 error, emit canonical status/retry shapes, and declare only capabilities actually implemented. |
...
| An SDK maintainer | Generate from the current signed RC.3 tarball, publish an exact support statement, and route accepted protocol corrections into a later prerelease. |
| A compliance operator | Pair `@adcp/sdk@14.0.0-rc.40` with RC.3. Python `adcp==8.0.0b14` and Go `adcp/v3@v3.2.1` remain paired with RC.1. Retain exact package and wire versions and do not issue a 3.2 badge from a prerelease checkpoint. |

### Required 3.2 migrations

- `create_media_buy` and `update_media_buy` success bodies use `media_buy_status`; root `status` is the task-envelope state. See [Migrating to `media_buy_status`](/dist/docs/3.2.0-rc.4/reference/migration/media-buy-status).
...
- 3.2 producers populate `error.recovery` on every error. The shared schema remains permissive for 3.1 compatibility; SDK consumers should qualify against the [error-recovery reference vectors](https://adcontextprotocol.org/compliance/latest/test-vectors/error-recovery/vectors.json).
...
- Idempotency replay is required for state-mutating requests, not pure reads. Read wrappers must tolerate an optional `idempotency_key`, but may ignore it; buyers cannot infer read-response replay from `adcp.idempotency.supported: true`.
- Product and signal agents may declare `anonymous_discovery` in their respective capability block. `true` permits an anonymous discovery response without claiming catalog completeness; `false` requires an authenticated principal; absence preserves legacy probe-and-handle behavior. Mutations and private or account-scoped reads remain authenticated by protocol rule rather than seller-variable capability.
...
New integrations SHOULD use the compact tools. Existing integrations do not need an immediate migration: the three compatibility facades retain their 3.x request shapes. On `get_products`, the key remains optional on every arm. A seller may ignore it for a guaranteed side-effect-free synchronous read. Buyers should supply it whenever the request may allocate a task or change observable state. When supplied and the seller declares `adcp.idempotency.supported: true`, the seller fences replay before that effect.
...
The legacy field remains schema-valid for 3.x wire compatibility. Consumers must not activate received credentials automatically: they may process the legacy form only for an explicitly allowlisted peer and otherwise must reject or quarantine it. The field is eligible for removal in 4.0 or later only after both deprecation-policy gates are satisfied: at least six months from publication of this notice and at least one full release cycle. See [Migrating secured asset access](/dist/docs/3.2.0-rc.4/reference/migration/asset-access) and [#5699](https://github.com/adcontextprotocol/adcp/issues/5699) for removal tracking.
...
- [`list_creatives`](/dist/docs/3.2.0-rc.4/creative/task-reference/list_creatives) remains the creative-library query task.
...
See [`list_creative_formats`](/dist/docs/3.2.0-rc.4/creative/task-reference/list_creative_formats), [Creative Protocol specification § Conformance](/dist/docs/3.2.0-rc.4/creative/specification#conformance), and [Implementing creative agents](/dist/docs/3.2.0-rc.4/creative/implementing-creative-agents).
...
**Status:** Published September 13, 2026 at 12:56 UTC — emergency privacy/security correction for the experimental Trusted Match Protocol (`trusted_match.core`). The stable wire negotiation value remains `"3.1"`.

Version 3.1.22 corrected two public Context Match privacy/security issues:

- [#7396](https://github.com/adcontextprotocol/adcp/pull/7396) prohibits dense embeddings derived from non-public content attributable to a single user or session and requires privacy reduction for other free-form context signals.
- [#7397](https://github.com/adcontextprotocol/adcp/pull/7397) replaces placement-only response caching with request-aware partitioning by `{provider_id, context_hash}`, rewrites `request_id` on cache hits, and keeps request-derived cache material out of observability surfaces.

The generated v3.1.22 changelog included #7396 but omitted #7397 because the 3.1.x backport did not carry its own changeset. Version 3.1.23 supplies that missing release coverage. The published [v3.1.22 release](https://github.com/adcontextprotocol/adcp/releases/tag/v3.1.22), tag, package, and versioned `dist` artifacts remain immutable.

### Immediate Context Match cache action

Context Match router operators affected by unsafe response caching MUST bypass or disable that caching until the router implements both request-aware partitioning and a trusted namespace for every result-affecting security and provider-configuration context. This includes authenticated principal or tenant, authorization and entitlement revision, active-package generation, provider endpoint and configuration, deployed model, and targeting or rules generation.

The request-aware `{provider_id, context_hash}` rule is the v3.1.22 correction. The additional `cache_namespace` conformance contract in main [#7446](https://github.com/adcontextprotocol/adcp/pull/7446) is 3.2-only and effective October 25, 2026; it is not retroactive 3.1 conformance. Until a router can establish both dimensions safely, bypassing cache lookup, coalescing, and insertion is the safe operational posture.

### Emergency notice exception

The v3.1.22 corrections shipped immediately as an emergency security exception to the normal six-week notice process for breaking changes to experimental surfaces. This records the exception; it does not change the general [experimental notice contract](/dist/docs/3.2.0-rc.4/reference/experimental-status#contract-for-experimental-surfaces). Corrections to the release record are carried forward in current-source documentation and the next patch rather than by rewriting published v3.1.22 artifacts.
...
For adoption-oriented framing before the full change list, start with [What's New in AdCP 3.1](/dist/docs/3.2.0-rc.4/reference/whats-new-in-3-1). For a role-based upgrade checklist, use [Migrating from 3.0 to 3.1](/dist/docs/3.2.0-rc.4/reference/migration/3-0-to-3-1).
...
| A house running the AAO crawler today (or any consumer that walks brand hierarchy) | Read [`brand.json` § Mutual-assertion trust model](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json#mutual-assertion-trust-model). The 3-tier read — identity-trusted vs relationship-trusted vs unverified — replaces the binary "is this in my portfolio" check for `brand_refs[]` pointer children. The 180-day TTL is already AAO's reference behavior. |
...
See [`brand.json` § Distributed publishing](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json) for the full normative spec including resolution algorithm, Conformance MUSTs, adoption path, and prior art.
...
**Creative-agent canonical build capabilities.** Creative agents declare what they can produce via `creative.supported_formats[]` on `get_adcp_capabilities`. Every targetable entry carries an agent-local stable `capability_id`; buyers pass that ID as `target_capability_id` in `build_creative`. This is intentionally separate from media-buy `format_option_id`, which selects a product-local or publisher-catalog format option. The training agent and storyboards verify advertised capability IDs, canonical `format_kind` manifests, unsupported target rejection, and 3.0 compatibility gating. See [Canonical formats](/dist/docs/3.2.0-rc.4/creative/canonical-formats) and [`build_creative`](/dist/docs/3.2.0-rc.4/creative/task-reference/build_creative).

**Canonical format satisfaction at create time.** Media-buy create/update/read surfaces now preserve package-level canonical selectors, including direct `format_kind`/`params` requests. The conformance suite tests that under-specified selectors fail closed instead of matching products by loose overlap. See [Product discovery](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/media-products).

**Per-version verification badge issuance.** Public verification runs issue badges for the selected public AdCP line. Badges are version-scoped, so 3.0 badges remain active independently of 3.1 badges. Buyers pinned to a specific AdCP release should read versioned badge data instead of flattening to a single current badge. See [Compliance catalog](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog) and [Versioning](/dist/docs/3.2.0-rc.4/reference/versioning).
...
A bundle of normative changes that addressed wrapper failures and attempted to extend replay to reads. The read-replay requirement was not implemented by the reference SDKs and is relaxed by the 3.2 mutation-only contract described above.

**`get_products` accepts optional keys (#4399b).** Sellers MUST accept `idempotency_key` on product discovery. The original 3.1 text required normal replay even for a pure-read result. AdCP 3.2 corrects that overreach: a guaranteed side-effect-free synchronous arm may ignore the key. Whenever a request may allocate a task or change observable state, buyers SHOULD supply a key and sellers declaring `adcp.idempotency.supported: true` MUST apply replay before the effect. The legacy facade keeps the field optional throughout 3.x.

A seller may voluntarily cache keyed reads, but buyers cannot rely on that behavior. Such sellers follow the full durability, rate-limit, and encryption contract and isolate read inserts from mutation replay capacity. See [security.mdx § Idempotency](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security#idempotency) for the current contract.

**MCP envelope serialization is now normative — flat on the wire (#2911).** `core/protocol-envelope.json` drops `required: [status, payload]`; envelope fields and body fields are siblings at the root of the MCP tool response (no nested `payload:` key). Matches shipping SDK behavior. `context` joins the envelope as a first-class caller-supplied opaque echo (`/schemas/core/context.json`). It is distinct from A2A's native `contextId`; a request-body `context_id` on MCP is compatibility-only where declared. A2A and REST serializations are normative in the envelope's `notes` array.

**MCP tool wrappers MUST tolerate schema-admitted envelope fields (#4399).** FastMCP/Pydantic strict signatures, Zod `.strict()`, and OpenAPI codegen with `additionalProperties: false` injected into otherwise-open input models are non-conformant. An admitted request-body `context_id` is accepted and ignored for compatibility. See [mcp-guide.mdx § Server-side tool wrappers](/dist/docs/3.2.0-rc.4/building/by-layer/L0/mcp-guide#server-side-tool-wrappers-must-tolerate-envelope-fields).

**`error.code` decoding is forward-compatible by contract (#4227).** Receivers MUST treat unknown `error.code` values as valid and read `error.recovery` (the normative wire carrier from 3.1 onward) when present. When a legacy error omits it, receivers use the registered classification for a known code and the bounded `transient` fallback for an unknown code. AdCP 3.1 senders SHOULD populate `error.recovery` on every error; 3.2 makes that a producer MUST. Sellers upgrading 3.0.x → 3.1 SHOULD audit error-emit paths before bumping the advertised `adcp_version`. See [error-handling.mdx § Forward-compatible decoding](/dist/docs/3.2.0-rc.4/building/by-layer/L3/error-handling#forward-compatible-decoding-normative).
...
**`refine[]` finalize-exclusivity (#4107).** If any `refine[]` entry on `get_products` has `action: 'finalize'`, ALL entries MUST be proposal-scoped finalize entries. Mixing finalize with non-finalize entries is rejected with `INVALID_REQUEST`. Multi-finalize is atomic across proposal_ids; sellers that can't guarantee atomicity reject with `INVALID_REQUEST` (or `MULTI_FINALIZE_UNSUPPORTED` for a more specific signal). See [refinement guide § Finalize is exclusive](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/refinement#finalize-is-exclusive-within-refine).
...
| If you are… | What you need to do |
|---|---|
| A buyer using `@adcp/client` or `adcp-py` | Nothing for pure reads. Both SDKs generate `idempotency_key` for requests they classify as mutating and omit it otherwise. |
...
| A seller on FastMCP/Pydantic, Zod `.strict()`, or OpenAPI codegen | Audit input models — they MUST accept every field admitted by the selected request schema. An admitted MCP `context_id` is accepted and ignored for compatibility. Use `extra='allow'` (Pydantic), drop `.strict()` (Zod), or fix codegen config (OpenAPI) for open input schemas. |
...
| A seller currently emitting unknown error codes (platform-specific or pre-3.1 additions) | Add `error.recovery` to every error envelope when adopting 3.1. This is a 3.1 SHOULD, so omission remains wire-valid; 3.2 producers MUST populate it. Receivers use the bounded `transient` fallback when an unknown code omits the field. |
...
See [Metric lifecycle](/dist/docs/3.2.0-rc.4/media-buy/media-buys/optimization-reporting#metric-lifecycle) for the full capability → commitment → optimization → delivery picture and [`kind: vendor_metric`](/dist/docs/3.2.0-rc.4/media-buy/conversion-tracking/#kind-vendor_metric) for the goal-shape reference.
...
`adcp_major_version` (the 3.0 integer field) and `adcp.major_versions` remain functional through all of 3.x — additive ship, no required changes for 3.0-conformant agents. The full migration table lives in [Versioning & Governance § Migration timeline](/dist/docs/3.2.0-rc.4/reference/versioning#migration-timeline).
...
**Compliance grader at 3.1 is advisory.** The grader runs the new `version_negotiation` universal storyboard and reports presence on requests and responses — see the [migration timeline](/dist/docs/3.2.0-rc.4/reference/versioning#migration-timeline) for the 3.1 → 3.2 → 4.0 cadence.

Spec PR [#3493](https://github.com/adcontextprotocol/adcp/pull/3493). RFC: [`specs/version-negotiation.md`](https://github.com/adcontextprotocol/adcp/blob/main/specs/version-negotiation.md). Full normative reference: [Versioning & Governance § Version negotiation](/dist/docs/3.2.0-rc.4/reference/versioning#version-negotiation).
...
| Implementing governance or TMP | Review the experimental-surface additions below. Per the [experimental-status contract](/dist/docs/3.2.0-rc.4/reference/experimental-status), additive changes are permitted within a patch release. |
...
- **URL canonicalization** — Now applies uniformly to brand.json agent URLs (`brand_agent_entry.url`, `brand_agent.url`, `rights_agent.url`). Two URLs differing only in case, default port, or percent-encoded unreserved characters compare equal during agent resolution. New reference page at [URL canonicalization](/dist/docs/3.2.0-rc.4/reference/url-canonicalization).
...
Per the [experimental-surface contract](/dist/docs/3.2.0-rc.4/reference/experimental-status), these surfaces accept additive changes within a patch release. Changes here do not affect agents that don't implement these protocols.
...
**Status:** General Availability | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)
...
1. **Trust surface: four cryptographic primitives for agent-to-agent ad buying** — Grouped by symmetry on the wire, so what's true of a request is true of a webhook:
   ...
   **Requests** — buyer → seller:
   ...
   - **RFC 9421 HTTP Message Signatures** — optional in 3.0, required for AdCP Verified. Ed25519 over a canonicalized covered-component list (including `content-digest`). sf-binary and URL canonicalization pinned so independent implementations produce bit-identical canonical inputs. Verifier follows a 15-step checklist (`keyid` cap-before-crypto, SSRF-validated JWKS fetch, `jti` replay dedup, audience binding) — see the [Security guide](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security). Published test vectors under `static/compliance/source/test-vectors/request-signing/`. (#2323, #2341, #2342, #2343)
...
- **Request signing (optional in 3.0, required for Verified)** — If you plan to claim AdCP Verified, implement RFC 9421 Ed25519 signing per the [signing profile](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security#request-signing) and declare your signing key via the account surface. Test against `static/compliance/source/test-vectors/request-signing/` and the runner's `signed_requests` harness.
...
- **Capabilities** — Remove all boolean capability checks (`features.content_standards`, `brand.identity`, `trusted_match.supported`, etc.). Test for object presence instead. See the [prerelease upgrade notes](/dist/docs/3.2.0-rc.4/reference/migration/prerelease-upgrades#capabilities-model-simplification) for the full field list.
- **Products** — Ensure all products include `reporting_capabilities`. This field is now required.
- **Media buy status** — Replace `pending_activation` with `pending_creatives` and `pending_start`. See the [state machine](/dist/docs/3.2.0-rc.4/media-buy/media-buys#state-machine).
...
- **Upgrading from v2?** Start with the [migration guide](/dist/docs/3.2.0-rc.4/reference/migration).
- **Upgrading from a prerelease?** Go to the [prerelease upgrade notes](/dist/docs/3.2.0-rc.4/reference/migration/prerelease-upgrades).
- **New to AdCP?** Read the [3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3) and then [get started](/dist/docs/3.2.0-rc.4/quickstart).

---

## Version 3.0.0-rc.3

**Status:** Release Candidate (superseded by 3.0.0) | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3) | [SDK support](/dist/docs/3.2.0-rc.4/building/schemas-and-sdks#adcp-3-0-support)
...
| Governance context | Structured `governance-context.json` schema | Signed-JWS `governance_context` string (opaque to forwarders, cryptographically verifiable by auditors — see [security.mdx](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security#signed-governance-context)) |
...
**Status:** Release Candidate | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)
...
13. **Proposal Refinement via Buying Mode** — `proposal_id` removed from `get_products` request. Refinement now uses `buying_mode: "refine"` with a typed `refine` array (see item 21). A2A uses native `contextId` for conversation correlation; MCP continuity, if provided, belongs to its transport session. Proposal execution via `create_media_buy` with `proposal_id` is unchanged.
...
**Status:** Beta | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)
...
4. **Commerce Media** - Catalog-driven product discovery (`catalog` on `get_products`), catalog-driven packages, per-catalog-item delivery reporting (`by_catalog_item`), store catchment targeting, and `catalog_types` on products. Cross-retailer GTIN matching via catalog selectors. Commerce attribution metrics (`roas`, `new_to_brand_rate`) in delivery response. See the [Commerce Media Guide](/dist/docs/3.2.0-rc.4/media-buy/commerce-media) and [Catalogs](/dist/docs/3.2.0-rc.4/creative/catalogs).
...
- Transport-native conversation correlation (`contextId` on A2A; transport session on MCP)

---

## Version 3.0.0-beta.2

**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta2) | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)
...
**Status:** Beta | [Full Changelog](https://github.com/adcontextprotocol/adcp/blob/main/CHANGELOG.md#300-beta1) | [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)
...
Version 3.0.0 is a **major release** that expands AdCP beyond media buying into governance, brand suitability, and conversational commerce. See the [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3) for detailed upgrade instructions.

**🎯 Key Themes:**

1. **Media Channel Taxonomy** - Complete overhaul from 9 format-oriented channels to 19 planning-oriented channels that reflect how buyers allocate budgets. See [Media Channel Taxonomy](/dist/docs/3.2.0-rc.4/reference/media-channel-taxonomy).
...
3. **Sponsored Intelligence Protocol** - Conversational brand experiences in AI assistants. Defines how hosts invoke brand agent endpoints for rich engagement without breaking conversational flow. See [Sponsored Intelligence](/dist/docs/3.2.0-rc.4/sponsored-intelligence/si-chat-protocol).
...
See [AdCP 3.0 overview](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3) for detailed before/after examples and migration steps.
...
See [SI Chat Protocol](/dist/docs/3.2.0-rc.4/sponsored-intelligence/si-chat-protocol) for complete documentation.
...
- [ ] Update channel enum values ([taxonomy guide](/dist/docs/3.2.0-rc.4/reference/media-channel-taxonomy))
...
**[View AdCP 3.0 overview →](/dist/docs/3.2.0-rc.4/reference/whats-new-in-v3)**
...
- `/schemas/3.2.0-rc.4/` - Backward compat alias (same as v2)
...
See [Versioning & Governance](/dist/docs/3.2.0-rc.4/reference/versioning) for the canonical version tiers, release cadence, 3.x stability guarantees, and deprecation policy. See the [v2 sunset page](/dist/docs/3.2.0-rc.4/reference/v2-sunset) for the v2 end-of-life timeline.
