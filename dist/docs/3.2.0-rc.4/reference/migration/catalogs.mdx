...
description: "Migrate AdCP catalogs from v2 to v3. Replaces promoted_offerings with first-class catalog objects, sync_catalogs task, and format-level catalog requirements."
"og:title": "AdCP — Migrating catalogs"
---
...
- **Format identity** is portable canonical `format_kind` plus an optional product or publisher `format_option_ref`; compound `format_id` is deprecated in 3.2
- **Catalogs** are `catalog` asset types in the `assets` map, keyed by the canonical slot name (for `sponsored_placement`, `source_catalog`)
...
```
get_products.format_options[] → resolve effective canonical catalog slots → sync_catalogs → sync_creatives with canonical manifests
```

1. Discover what catalogs a product contract needs by resolving the selected canonical's default slots plus any narrowing or override in `get_products.format_options[].params.slots`
...
<Card title="Catalogs documentation" icon="arrow-right" href="/dist/docs/3.2.0-rc.4/creative/catalogs">
...
<Card title="Conversion events" icon="arrow-right" href="/dist/docs/3.2.0-rc.4/creative/catalogs#conversion-events">
...
<Steps>
  ...
  <Step title="Discover catalog requirements">
    Resolve the selected declaration's effective canonical slots — canonical defaults plus any `format_options[].params.slots` narrowing or override — and check for catalog inputs. Use `get_adcp_capabilities.creative.supported_formats[]` only when selecting a separate creative agent to build or validate the resulting manifest; `list_creative_formats` is deprecated in 3.2.
  </Step>
