...
<Note>
Some names like `dooh`, `podcast`, `radio`, and `streaming_audio` appear as both a channel and a property type. The channel describes the **buying context** (how budget is allocated); the property type describes the **technical surface and its operator**. For example, the same venue-operated audio estate can declare `supported_channels: ["dooh", "streaming_audio", "retail_media"]`, while each product narrows `channels` to the buying context in which that product is sold.
</Note>
...
| Venue audio network | `dooh`, `streaming_audio`, `retail_media` | One operated endpoint estate sold through different budget lines |
...
For audio inventory, execution and distribution details that sit below channel and property type belong on product or placement metadata. Use [`audio_distribution_types`](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/media-products#audio-distribution-types) to distinguish music streaming services, FM/AM broadcast, podcasts, catch-up radio, web radio, video-game audio, and text-to-speech without renaming buyer-facing channels or `adagents.json` property types. The field describes the feed, not who operates the endpoint; a venue-operated audio player uses the `dooh` property type even when its property supports more than one buyer-facing channel.
...
| `dooh` | Digital out-of-home buying context, including venue-operated screens and audio players |
...
Buyer-facing allocation category for digital out-of-home inventory. A property offered in this channel can contain venue-operated visual, audio-only, or multimodal endpoints; the canonical creative format determines the medium.
...
- Venue-operated audio players in taverns, stores, gyms, hotels, and other managed venues

**Excludes**:
- Static billboards (use `ooh`)
- Cinema screens (use `cinema`)
- Audio products sold only through a personal-streaming budget line (use `streaming_audio`)
- Terrestrial radio broadcasts (use `radio`)

**Typical Formats**: display, video, audio
...
**Note**: `affiliate` describes the BUYING MODEL (performance-based, CPA/CPC/revenue share) rather than where content appears. Percentage-of-sale terms use the [`revenue_share` pricing model](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/pricing-models#revenue-share).
...
| `dooh` | Venue-operated digital endpoint networks | `dooh`, `streaming_audio`, `retail_media` |
...
With `channels` on products, buyers can distinguish retail media product types (sponsored products vs display vs offsite) without sub-channels. Use `channels: ["retail_media"]` combined with canonical `format_kinds` or `format_option_refs` filters to narrow results. The two-axis approach (channel + format) is preferred over channel proliferation.
