---
title: Catalogs
description: "Catalogs in AdCP are typed data feeds that supply items for ad rendering across platforms."
"og:title": "AdCP — Catalogs"
testable: true
---

# Catalogs

Catalogs are all the ingredients you give sellers so they can assemble great outcomes for your brand. An e-commerce brand pushes its product catalog — SKUs, prices, images — so platforms can build sponsored product carousels. A travel advertiser pushes flights, hotels, and destination guides. An employer pushes job openings. A retailer pushes store locations and real-time inventory. A brand pushes its offerings, services, and seasonal campaigns.

The richer the ingredients, the better the result. On AI platforms especially, your catalog data IS the creative input — the platform's LLM assembles ads from your products, offerings, and brand context rather than serving a pre-built banner. But catalogs matter beyond AI media too: any seller that dynamically assembles ads from your data — retail media networks, commerce platforms, native ad networks — benefits from richer catalog feeds.

Catalogs also connect directly to measurement. Push your store catalog and a seller can correlate ad delivery to store-level foot traffic. Push your product catalog and a retail media network can close the loop on purchase attribution. The same data that enables better creative also enables better measurement.
...
- **Buyers can immediately suppress or restore items** in their synced catalogs through `sync_catalogs.item_availability_updates`, without waiting for the next scheduled feed fetch
...
| I'm advertising... | Catalog type | Item schema | Required fields |
|---|---|---|---|
| Physical products / ecommerce SKUs | `product` | *(your existing feed — Google Merchant, Shopify, etc.)* | Depends on feed format |
| Job openings / recruitment | `job` | [JobItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#job-catalog) | `job_id`, `title`, `company_name`, `description` |
| Hotels / lodging | `hotel` | [HotelItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#hotel-catalog) | `hotel_id`, `name`, `location` |
| Vehicles (new or used) | `vehicle` | [VehicleItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#vehicle-catalog) | `vehicle_id`, `title`, `make`, `model`, `year` |
| Flights / air travel | `flight` | [FlightItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#flight-catalog) | `flight_id`, `origin`, `destination` |
| Real estate listings | `real_estate` | [RealEstateItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#real-estate-catalog) | `listing_id`, `title`, `address` |
| University programs / courses | `education` | [EducationItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#education-catalog) | `program_id`, `name`, `school` |
| Travel destinations | `destination` | [DestinationItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#destination-catalog) | `destination_id`, `name` |
| Mobile apps | `app` | [AppItem](/dist/docs/3.2.0-rc.4/creative/catalog-schemas#app-catalog) | `app_id`, `name`, `platform` |
| Store locations | `store` | [StoreItem](#stores) | `store_id`, `name`, `location` |
| Campaigns, services, events, asset groups | `offering` | [Offering](#offerings) | `offering_id`, `name` |
...
Each vertical type (job through app) has a defined AdCP schema with examples — see the [catalog item schemas](/dist/docs/3.2.0-rc.4/creative/catalog-schemas) page for field tables and starter templates. Structural types (product, inventory, promotion) use freeform schemas that map to your existing feed format via `feed_format` and `feed_field_mappings`.
...
That ownership direction also governs real-time availability. When an item in a buyer-managed feed becomes unavailable or available again, the buyer can push an immediate `suppress` or `restore` overlay through [`item_availability_updates`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/sync_catalogs#immediate-item-availability). This is distinct from seller-owned wholesale inventory and from seller-initiated policy withdrawal. A buyer restore never overrides the seller's review or eligibility controls.
...
sequenceDiagram
    ...
    rect rgb(240, 248, 255)
        ...
        B->>S: get_products (read format_options)
...
This is the same dependency order documented in [Account state](/dist/docs/3.2.0-rc.4/building/by-layer/L2/account-state): `sync_catalogs` → `sync_creatives` → `create_media_buy`.
...
|------|-------------|-------------|
| `offering` | [Offering](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/offering.json) | AdCP Offering objects — campaigns, vacancies, events, services |
...
| `store` | [StoreItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/store-item.json) | Physical locations with addresses and catchment areas |
...
|------|-------------|---------|
| `hotel` | [HotelItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/hotel-item.json) | Google Hotel Center, Meta hotel catalogs |
| `flight` | [FlightItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/flight-item.json) | Google DynamicFlightsAsset, Meta flight catalogs |
| `job` | [JobItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/job-item.json) | LinkedIn Jobs XML, Google DynamicJobsAsset, schema.org JobPosting |
| `vehicle` | [VehicleItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/vehicle-item.json) | Meta Automotive Inventory, Microsoft Auto Inventory feeds |
| `real_estate` | [RealEstateItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/real-estate-item.json) | Google DynamicRealEstateAsset, Meta home listing catalogs |
| `education` | [EducationItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/education-item.json) | Google DynamicEducationAsset, schema.org Course |
| `destination` | [DestinationItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/destination-item.json) | Meta destination catalogs, Google travel ads |
| `app` | [AppItem](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/app-item.json) | Google App Campaigns, Apple Search Ads, Meta App Ads, TikTok App Campaigns, Snapchat App Install Ads |
...
**Schema URL**: [`/schemas/core/catalog.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/catalog.json)
...
Each vertical catalog type has a defined item schema with field tables, required fields, and starter templates. See **[Catalog item schemas](/dist/docs/3.2.0-rc.4/creative/catalog-schemas)** for the complete reference with minimal and full examples for every vertical type.

## Format catalog requirements

Canonical declarations that render product listings, store locators, or promotional content declare catalog inputs in `params.slots[]`. This tells buying agents which catalogs to sync before submitting creatives.

```json
{
  "format_option_id": "product_carousel_with_inventory",
  "format_kind": "image_carousel",
  "params": {
    "slots": [{
      "asset_group_id": "product_catalog",
      "asset_type": "catalog",
      "required": true,
      "requirements": {
      ...
      "asset_group_id": "inventory_catalog",
      "asset_type": "catalog",
      "required": true,
      "requirements": {
...
Buying agents check catalog slots in the selected product's `format_options[]`, sync the required feeds via `sync_catalogs`, then submit canonical creatives that reference them.
...
1. **Discover format requirements** — Read the selected product's canonical `format_options[]` and check `params.slots[]` for catalog assets and constraints.
  ...
  "creatives": [
    {
      ...
      "format_kind": "image_carousel",
      "format_option_ref": { "scope": "product", "format_option_id": "product_carousel_with_inventory" },
      "assets": {
        "product_catalog": {
          "asset_type": "catalog",
          "catalog_id": "product-feed",
...
**Schema URL**: [`/schemas/core/offering.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/offering.json)
...
type PostalArea =
  | {
      ...
      system: string;   // Country-local postal system, e.g. 'zip', 'plz', 'postal_code'
      values: string[];
    }
  | {
      system: string;   // Deprecated country-fused alias, e.g. 'us_zip'
      values: string[];
    };
...
**Schema URL**: [`/schemas/core/offering-asset-group.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/offering-asset-group.json)
...
**Schema URL**: [`/schemas/core/requirements/offering-asset-constraint.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/requirements/offering-asset-constraint.json)
...
Read the selected canonical declaration's `params.slots[]` for catalog assets to see what catalog types are needed and what each offering must provide:
...
**Schema URL**: [`/schemas/core/store-item.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/store-item.json)
...
**Schema URL**: [`/schemas/core/catchment.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/core/catchment.json)
...
Offerings in a catalog can be promoted through Sponsored Intelligence conversations. The brand's SI agent URL is declared on the [brand identity](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json), not on the catalog — SI is a brand-level capability.
...
No pre-built creatives — just offerings available for SI conversations. The brand's SI agent URL is discovered from the [brand identity](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json):
...
Pass a `catalog` on [`get_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_products) to find products that match your catalog items. The seller matches catalog items against its inventory and returns products where matches exist. Products declare which catalog types they support via `catalog_types`.

### Catalog-driven packages

Include a `catalog` field on a package in [`create_media_buy`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/create_media_buy) to make it catalog-driven. One budget envelope promotes the entire catalog — the platform optimizes delivery across items based on performance. This is the AdCP equivalent of catalog-based campaign types like Google Performance Max or Meta Dynamic Product Ads.

### Variants as catalog items

For catalog-driven packages, each catalog item rendered as a distinct ad execution is a [creative variant](/dist/docs/3.2.0-rc.4/creative/task-reference/get_creative_delivery). The variant's manifest includes the catalog reference with the specific item rendered.

### Per-item delivery reporting

[`get_media_buy_delivery`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) returns `by_catalog_item` breakdowns within each package, showing per-item impressions, spend, clicks, conversions, and ROAS.

### Conversion attribution

Conversion events carry `content_ids` that identify which catalog items were involved. The catalog's `content_id_type` declares the identifier type. Attribution is broad — a user might click item A but convert on item B. The event fires with the actual `content_id`. See [conversion tracking](/dist/docs/3.2.0-rc.4/media-buy/conversion-tracking).

### Item-level tracking via macros

For impression-level attribution (which item was clicked, which was viewed), use [catalog item macros](/dist/docs/3.2.0-rc.4/creative/universal-macros#catalog-item-macros) in your creative's tracker pixel URLs. The macros mirror the `content_id_type` enum — the same identifiers used at serve time that appear in conversion events:
...
The macros above resolve to the rendered item's **identifier**. Catalog **content** macros resolve to a rendered item's scalar **field values**, for catalog-driven creative templates (`sponsored_placement` / DPA — Meta DPA, Snap Collection, TikTok Shopping). Each token maps 1:1 to a catalog item field via the same `catalog_field` dot-notation paths used by [`field_bindings`](#field-bindings) — content macros reuse that vocabulary rather than defining a parallel one:

| Macro | `catalog_field` |
|-------|-----------------|
| `{ITEM_NAME}` | `name` |
| `{ITEM_DESCRIPTION}` | `description` |
| `{ITEM_TAGLINE}` | `tagline` |
| `{ITEM_PRICE}` | `price.amount` |
| `{ITEM_PRICE_CURRENCY}` | `price.currency` |

Content macros use single-brace `{MACRO}` syntax and substitute under the same [substitution-safety rules](/dist/docs/3.2.0-rc.4/creative/universal-macros#substitution-safety-catalog-item-macros) as the ID macros (NFC normalization → RFC 3986 percent-encoding → one-pass nested-expansion prohibition → URL-context scope). `{{double-brace}}` is **not** a content-macro syntax — it stays reserved for downstream ad-server macros the sales agent must neutralize. URL-valued and image-valued fields (`landing_url`, image pools) bind via `field_bindings` to `url`/asset slots, not content macros.

Which items render is **seller-declared** via the `fanout_mode` enum on the [`sponsored_placement`](/dist/docs/3.2.0-rc.4/creative/canonical-formats) format (`single_item` for the Meta-DPA one-item-per-ad pattern, `per_item`, or `multi_item_in_creative`) — there is no buyer-side selection field. On ML-optimized DPA surfaces the platform may override buyer-authored overlay text, so content macros are a hint the seller MAY honor.

## Best practices

### Match `asset_group_id` to the format's vocabulary

Read the selected product or creative capability's canonical declaration before building offerings. The `asset_group_id` values must match exactly what the declaration carries in `offering_asset_constraints` within each catalog slot's requirements. Group IDs like `headlines`, `images`, or `videos` are declaration-defined vocabulary, not protocol constants.
...
- **[Account state](/dist/docs/3.2.0-rc.4/building/by-layer/L2/account-state)** — How catalogs fit into account state and the setup sequence
- **[Brand identity](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json)** — Brand identity and assets
- **[SI Chat Protocol](/dist/docs/3.2.0-rc.4/sponsored-intelligence/si-chat-protocol)** — Conversational brand experiences
- **[sync_creatives](/dist/docs/3.2.0-rc.4/creative/task-reference/sync_creatives)** — Submitting creative manifests
- **[Canonical formats](/dist/docs/3.2.0-rc.4/creative/canonical-formats)** — Discovering format requirements and authority
