...
Now the signals are live on both platforms. Sam's agent builds media buys using [`create_media_buy`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/create_media_buy). The activated signals are already available as targeting segments on each DSP — the media buy references the products discovered via [`get_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_products), and the DSP applies the signal-based targeting automatically.
...
The key point: signals and media buys are separate concerns. The Signals Protocol gets data onto platforms. The [Media Buy Protocol](/dist/docs/3.2.0-rc.4/media-buy/index) gets campaigns running on those platforms. They compose together — Sam didn't need a custom integration between his signal providers and his DSPs.
...
- **Key concepts**: [Signal types, sources, and authorization](/dist/docs/3.2.0-rc.4/signals/key-concepts) — the building blocks behind this walkthrough
- **Ecosystem**: [Who participates and how](/dist/docs/3.2.0-rc.4/signals/ecosystem) — data providers, retailers, publishers, CDPs, agencies, identity companies
- **Publish your data**: [Data provider guide](/dist/docs/3.2.0-rc.4/signals/data-providers) — how to create published signal definitions
- **Protocol spec**: [Signals specification](/dist/docs/3.2.0-rc.4/signals/specification) — formal requirements and conformance
- **Get certified**: The [Signals specialist module](/dist/docs/3.2.0-rc.4/learning/specialist/signals) teaches signal discovery and activation through interactive labs with a sandbox signal agent
