...
description: "activate_signal is the AdCP task for pushing audience segments to DSPs and sales agents. Supports async activation, deactivation for data governance, and returns platform-specific activation keys."
"og:title": "AdCP — activate_signal"
---
...
**Request Schema**: [`https://adcontextprotocol.org/schemas/3.2.0-rc.4/signals/activate-signal-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/signals/activate-signal-request.json)
**Response Schema**: [`https://adcontextprotocol.org/schemas/3.2.0-rc.4/signals/activate-signal-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/signals/activate-signal-response.json)
...
|-----------|------|----------|-------------|
| `idempotency_key` | string | Yes | Client-generated unique key for this request. Prevents duplicate activations on retries. MUST be unique per (seller, request) pair. Min 16 chars. See [Idempotency](/dist/docs/3.2.0-rc.4/building/by-layer/L1/security#idempotency) for normative semantics. |
| `signal_agent_segment_id` | string | Yes | Opaque signal handle from [`get_signals`](/dist/docs/3.2.0-rc.4/signals/tasks/get_signals) for the signal to activate |
...
| `account` | [AccountRef](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents#account-references) | No | Account for this activation. Associates with a commercial relationship established via [`sync_accounts`](/dist/docs/3.2.0-rc.4/accounts/tasks/sync_accounts). |
| `pricing_option_id` | string | Yes (if signal has pricing options) | The pricing option selected from `pricing_options` in the [`get_signals`](/dist/docs/3.2.0-rc.4/signals/tasks/get_signals) response. Records the buyer's pricing commitment at activation time. Pass this same value in subsequent [`report_usage`](/dist/docs/3.2.0-rc.4/accounts/tasks/report_usage) calls. |
| `governance_context` | string | Conditional | Opaque approval context returned by [`check_governance`](/dist/docs/3.2.0-rc.4/governance/campaign/tasks/check_governance) for this activation. Required when the activation account has a registered governance agent. |

## Governance

Signal activation is a spend-commit event when the selected signal carries activation, usage, or CPM fees. For governed accounts, the buyer-side orchestrator MUST call [`check_governance`](/dist/docs/3.2.0-rc.4/governance/campaign/tasks/check_governance) before `activate_signal` with `purchase_type: "signal_activation"` and a normalized activation payload that includes the spend basis, such as `total_budget` or `budget`.

When `check_governance` approves the activation, pass the returned `governance_context` into `activate_signal`. If the account has a governance agent registered via [`sync_governance`](/dist/docs/3.2.0-rc.4/accounts/tasks/sync_governance) and the request omits a valid `governance_context`, the signal agent MUST reject the request with [`PERMISSION_DENIED`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-permission-denied). If `check_governance` denies the activation, the buyer MUST NOT call `activate_signal`.
...
```json
{
  "$schema": "/schemas/3.2.0-rc.4/signals/activate-signal-response.json",
  "status": "completed",
...
A non-streaming `SendMessage` response selects the completed Task branch. A
client that needs transport progress uses the official `SendStreamingMessage`
or `SubscribeToTask` operation and consumes `StreamResponse` SSE frames.
...
```json
{
  "task": {
    ...
    "artifacts": [{
      ...
      "parts": [
        { "text": "Signal successfully activated on The Trade Desk" },
        { "data": {
          "status": "completed",
          "deployments": [{
            "type": "platform",
...
- **A2A**: Returns the typed result in a completed Task; clients may opt into the official streaming operations for transport progress
...
See **[Webhooks](/dist/docs/3.2.0-rc.4/building/by-layer/L3/webhooks)** for complete details on webhook configuration and reliability.
...
### Activation Errors
- [`REFERENCE_NOT_FOUND`](/dist/docs/3.2.0-rc.4/building/verification/compliance-catalog#error-code-reference-not-found): Referenced `signal_agent_segment_id` doesn't exist or is not accessible to the calling account (`error.field` identifies the failing parameter)
