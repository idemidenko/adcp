...
description: "How AI advertising agents discover inventory, exchange campaign data, and execute buys across platforms using AdCP's standardized task schemas and MCP transport."
"og:title": "AdCP — How AI agents communicate ad specs across platforms"
---
...
```json
{
  "idempotency_key": "550e8400-e29b-41d4-a716-446655442007",
  "buying_mode": "brief",
...
Before submitting creatives, the agent reads the product's canonical `format_options[]`. The declaration is already complete: it carries the accepted canonical kind, parameters, and asset slots.

```json
{
  "format_options": [
    {
      "format_option_id": "video_preroll_16x9",
...
When production is needed, the buyer finds a compatible creative agent through the registry and selects one of that agent's `creative.supported_formats[]` entries with `build_creative.target_capability_id`.
...
The agent compares the products' canonical `format_options[]` declarations and identifies a compatible shared constraint envelope across all three, reducing creative production to a shared set.
...
<CardGroup cols={2}>
  ...
  <Card title="Media buy protocol" icon="robot" href="/dist/docs/3.2.0-rc.4/media-buy/index">
