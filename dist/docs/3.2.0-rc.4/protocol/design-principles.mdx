...
[SDKs](/dist/docs/3.2.0-rc.4/building/by-layer/L4/choose-your-sdk) implement them once for many
...
→ Use the [Capabilities explorer](/dist/docs/3.2.0-rc.4/protocol/capabilities-explorer) before proposing a new flag. It walks the actual schema tree.
...
published offer carries versioned pricing from [`list_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/list_products) into
[`buy_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/buy_products); seller-specific pricing or configuration moves through
[`request_proposals`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/request_proposals), typed [`refine_proposals`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/refine_proposals), a committed hold, and
[`accept_proposal`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/accept_proposal). A quote task would duplicate an existing commercial state
...
[`list_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/list_products). A buyer asking
the seller to plan sends a brief plus the same structured criteria to
[`request_proposals`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/request_proposals). In both
...
→ See [Targeting](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/targeting) for the brief-first model.
...
→ The [Capabilities explorer](/dist/docs/3.2.0-rc.4/protocol/capabilities-explorer) renders the existing tree.

---

### 5. Trust is bilateral and `/.well-known`-rooted

Trust in AdCP is bilateral and verifiable, not blessed by a registry. Operators declare organization identity, agents, property relationships, and signing-key discovery via [`brand.json`](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json) at `/.well-known/brand.json`; publishers declare which agents are authorized for which properties via [`adagents.json`](/dist/docs/3.2.0-rc.4/governance/property/adagents). Either party can resolve and verify the other's declarations. The [Registry](/dist/docs/3.2.0-rc.4/registry) helps with discovery and resolution; it does not gate participation. Every discovery hop is an HTTP GET against a deterministic path — the model ads.txt and sellers.json got right, deliberately reused.
...
→ See [Architecture — Privacy posture across domains](/dist/docs/3.2.0-rc.4/protocol/architecture#privacy-posture-across-domains) for the per-domain mechanism table.
...
- *Capabilities are declared, not gated.* [`check_governance`](/dist/docs/3.2.0-rc.4/governance/campaign/tasks/check_governance) is a seam, not an enforcer. A seller that hasn't configured a governance agent will not call it; the protocol doesn't prevent a non-conformant seller from transacting. Schema-level enforcement exists but is rare and named (`fair_housing`, `fair_lending`, `fair_employment` in 3.0); the default is exposure, not coercion.
...
→ See [Trust & Security — Governance](/dist/docs/3.2.0-rc.4/trust#governance) for what AdCP provides versus what it explicitly does not, and [Governance — Embedded human judgment](/dist/docs/3.2.0-rc.4/governance/embedded-human-judgment) for how HITL surfaces in the protocol.
...
→ The same anti-centralization instinct as principle 5, applied to reference data instead of trust: the [Registry](/dist/docs/3.2.0-rc.4/registry) helps resolve and discover, it does not become the ecosystem's lookup table.
