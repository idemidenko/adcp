...
description: "AdCP capability discovery — understand what creative formats a seller supports and which properties they represent before buying media."
"og:title": "AdCP — Capability Discovery"
---
...
### [Implementing Standard Format Support](/dist/docs/3.2.0-rc.4/media-buy/capability-discovery/implementing-standard-formats) 🎨
...
### [Understanding Authorization](/dist/docs/3.2.0-rc.4/governance/property/authorized-properties) 🔐
...
<Note>
**Cross-Protocol Foundation**: The `adagents.json` authorization system applies across ALL AdCP protocols (Media Buy, Signals, and future Curation). See the [adagents.json specification](/dist/docs/3.2.0-rc.4/governance/property/adagents) for complete implementation details.
</Note>
...
### Canonical format sources
Read seller deliverability from `get_products` `Product.format_options[]`, publisher acceptance from `adagents.json.formats[]`, and creative production capabilities from `get_adcp_capabilities.creative.supported_formats[]`. `list_creative_formats` is deprecated in 3.2.

## Integration Pattern

Capability discovery typically happens early in your AdCP workflow:

1. **Discover Capabilities**: Call [`get_adcp_capabilities`](/dist/docs/3.2.0-rc.4/protocol/get_adcp_capabilities) to learn what the agent supports
2. **Understand Formats**: Read canonical `format_options[]` on returned products and resolve publisher declarations when referenced
3. **Validate Authorization**: Check the `media_buy.portfolio` from capabilities, then verify via publisher `adagents.json`
4. **Discover Products**: Search for advertising inventory with [`get_products`](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_products)
5. **Plan Creatives**: Match product declarations to creative-agent `supported_formats[]` capabilities
...
- **[Task Reference](/dist/docs/3.2.0-rc.4/media-buy/task-reference/)** - Complete API documentation
- **[Product Discovery](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/)** - Finding advertising inventory
- **[Creatives](/dist/docs/3.2.0-rc.4/media-buy/creatives/)** - Creative asset management
- **[Creative Protocol](/dist/docs/3.2.0-rc.4/creative/)** - Creative agents and manifests
- **[Creative Channel Guides](/dist/docs/3.2.0-rc.4/creative/channels/video)** - Format examples and patterns
- **[Creative Manifests](/dist/docs/3.2.0-rc.4/creative/creative-manifests)** - Understanding creative specifications
- **[adagents.json Specification](/dist/docs/3.2.0-rc.4/governance/property/adagents)** - Publisher authorization system (applies across all AdCP protocols)
