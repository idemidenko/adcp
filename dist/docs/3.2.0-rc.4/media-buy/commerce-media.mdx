...
|---|---|---|
| Retailer / retail media network | Sales agent (MCP server) | [`adagents.json`](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/accounts-and-security) |
| Advertiser account with retailer | `account_id` + `list_accounts` | [Accounts & Agents](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents) |
| Brand's product catalog | `brand.product_catalog` | [Brand identity](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json) |
| GTIN / SKU selection for promotion | Catalog selectors (`ids`, `gtins`, `tags`) on `Catalog` | [Catalogs](/dist/docs/3.2.0-rc.4/creative/catalogs) |
| Sponsored product listing | Product with catalog-rendered creative | [Creative Formats](/dist/docs/3.2.0-rc.4/creative/formats) |
| On-site display / video | Product with canonical `format_options` | [Media Products](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/media-products) |
| Retailer-owned shopper signal | Brief-based; optional named segments discovered in `signal_targeting_options` and selected with package-level `signal_targeting_groups` | [Targeting](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/targeting) |
| ROAS target / max conversions | `optimization_goals` on package | [Optimization & Reporting](/dist/docs/3.2.0-rc.4/media-buy/media-buys/optimization-reporting) |
| Closed-loop measurement | `outcome_measurement` + `conversion_tracking` on product | [Delivery Reporting](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) |
| Attribution window (14d click, 1d view) | `attribution_window` in delivery response | [Delivery Reporting](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) |
| ROAS / attributed revenue | `roas`, `conversion_value` in delivery metrics | [Delivery Reporting](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) |
| In-store attribution | `by_action_source` with `in_store` | [Delivery Reporting](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) |
| Store locations / store locator | Store catalog (seller-provided or buyer-synced) | [Catalogs — Stores](/dist/docs/3.2.0-rc.4/creative/catalogs#stores) |
| Real-time inventory / in-stock | Inventory catalog via `sync_catalogs` | [Catalogs](/dist/docs/3.2.0-rc.4/creative/catalogs) |
| Proximity / catchment targeting | Store catchment areas (isochrone, radius, GeoJSON) | [Catalogs — Catchment areas](/dist/docs/3.2.0-rc.4/creative/catalogs#catchment-areas) |

Each retailer is a separate sales agent. Their media offerings are modeled as products. The buyer's brand identity carries the product catalog for SKU-level creative rendering. Account relationships between brands and retailers are managed via `list_accounts` and `account` on media buys — see [Accounts & Agents](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents).
...
```json
{
  ...
  "format_options": [
    { "format_option_id": "sponsored_product_listing", "format_kind": "sponsored_placement", "params": {} }
  ],
...
```json
{
  ...
  "placements": [
    {
      "kind": "seller_inline",
      "seller_agent": { "agent_url": "https://ads.shopgrid.example/adcp" },
      "placement_id": "homepage_hero",
      "name": "Homepage Hero Banner",
      "mode": "targetable",
      "format_options": [{ "format_option_id": "display_970x250", "format_kind": "image", "params": { "width": 970, "height": 250 } }]
    },
    {
      "kind": "seller_inline",
      "seller_agent": { "agent_url": "https://ads.shopgrid.example/adcp" },
...
```json
{
  ...
  "format_options": [
    { "format_option_id": "display_1080x1920", "format_kind": "image", "params": { "width": 1080, "height": 1920 } },
    { "format_option_id": "video_15s", "format_kind": "video_hosted", "params": { "duration_ms_exact": 15000 } }
  ],
  "placements": [
    {
...
Before creating media buys, sync the brand's data feeds to the retailer's account via `sync_catalogs`. This builds account state that creatives and targeting reference at serve time. See [Account state](/dist/docs/3.2.0-rc.4/building/by-layer/L2/account-state) for how this fits the broader setup sequence.
...
<Tip>
Sellers can also return [proposals](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/media-products#proposals) with recommended budget allocations across product types — encoding media planning expertise that traditionally required human sales reps.
</Tip>
...
…are equivalent to declaring `conversion_value` (the standardized name for "attributed sales") in `available_metrics` plus the matching qualifier on `committed_metrics`. Migrate by populating both fields during the transition window; remove `outcome_measurement` at the next major.
...
- [Account state](/dist/docs/3.2.0-rc.4/building/by-layer/L2/account-state) — How catalogs, event sources, and campaigns build on the account
- [Accounts & Agents](/dist/docs/3.2.0-rc.4/building/by-layer/L2/accounts-and-agents) — Account setup and billing relationships
- [Media Channel Taxonomy](/dist/docs/3.2.0-rc.4/reference/media-channel-taxonomy) — `retail_media` channel definition
- [Media Products](/dist/docs/3.2.0-rc.4/media-buy/product-discovery/media-products) — Product model reference
- [Catalogs](/dist/docs/3.2.0-rc.4/creative/catalogs) — Product, inventory, store, and promotion feeds
- [Brand identity](/dist/docs/3.2.0-rc.4/brand-protocol/brand-json) — Product catalog and brand identity
- [Pricing Models](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/pricing-models) — CPC, CPM, flat rate details
- [Optimization & Reporting](/dist/docs/3.2.0-rc.4/media-buy/media-buys/optimization-reporting) — `optimization_goals` and conversion optimization
- [Delivery Reporting](/dist/docs/3.2.0-rc.4/media-buy/task-reference/get_media_buy_delivery) — Commerce attribution metrics
- [Targeting](/dist/docs/3.2.0-rc.4/media-buy/advanced-topics/targeting) — Brief-based targeting approach
