...
- **Request**: [`get-brand-identity-request.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/brand/get-brand-identity-request.json)
- **Response**: [`get-brand-identity-response.json`](https://adcontextprotocol.org/schemas/3.2.0-rc.4/brand/get-brand-identity-response.json)
...
Authorized callers (linked via [`sync_accounts`](/dist/docs/3.2.0-rc.4/accounts/tasks/sync_accounts)) get deeper data on top of the public baseline.

Account linking is a one-time setup: a buyer agent calls [`sync_accounts`](/dist/docs/3.2.0-rc.4/accounts/tasks/sync_accounts) on the brand agent, providing its brand reference. After that, the buyer's `get_brand_identity` requests are recognized as authorized.
...
```json Response (authorized)
{
  ...
  "voice_synthesis": {
    "provider": "elevenlabs",
    "voice_id": "janssen_v2",
    "rights_offering_id": "loti_dj_voice_2026",
...
| `voice_synthesis` | object | No | TTS voice synthesis configuration (`provider`, `voice_id`, `settings`, optional `rights_offering_id`). The offering ID is advisory provenance from [`get_rights`](/dist/docs/3.2.0-rc.4/brand-protocol/tasks/get_rights), not a buyer-specific grant; resolve it through [`acquire_rights`](/dist/docs/3.2.0-rc.4/brand-protocol/tasks/acquire_rights) before synthesis. |
...
<CardGroup cols={2}>
  <Card title="get_rights" icon="scale-balanced" href="/dist/docs/3.2.0-rc.4/brand-protocol/tasks/get_rights">
