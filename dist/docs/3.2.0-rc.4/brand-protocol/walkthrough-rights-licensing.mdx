...
```json
{
  ...
  "campaign": {
    ...
    "uses": ["likeness", "voice"],
    "countries": ["NL"],
    "format_kinds": ["video_hosted", "image"],
...
```json
{
  ...
  "rights_constraint": {
    ...
    "content_digest": "sha256:4feebc941c805632ab259451fd1cda47fc56985dc62e177844ebfd377556ab7a",
    "attestation_refs": [{
      ...
      "subject": {
        ...
        "id": "loti_dj_talent_2026",
        "content_digest": "sha256:4feebc941c805632ab259451fd1cda47fc56985dc62e177844ebfd377556ab7a"
      },
      "locator": { "type": "issuer_credential_id", "credential_id": "cred_loti_dj_2026", "resolver_id": "primary" }
    }]
  }
}
```
...
<CardGroup cols={2}>
  <Card title="Try it yourself" icon="graduation-cap" href="/dist/docs/3.2.0-rc.4/learning/tracks/buyer">
...
Every creative that uses licensed talent carries the `rights_constraint` from the `acquire_rights` response unchanged — the buyer does not construct or re-sign it. A single ad can combine talent likeness from one rights holder and music from another, each with different validity periods and geographic restrictions. The serving seller evaluates each issuer-bound reference under its own allowlist and binds the result to the exact grant digest. A deprecated `verification_url`, if present on legacy data, is ignored for authorization and is never fetched as a revocation oracle.
...
<CardGroup cols={2}>
  ...
  <Card title="For rights holders" icon="shield-halved" href="/dist/docs/3.2.0-rc.4/brand-protocol/for-rights-holders">
    How AdCP protects and monetizes talent rights.
  </Card>
  <Card title="Building a brand agent" icon="robot" href="/dist/docs/3.2.0-rc.4/brand-protocol/building-a-brand-agent">
    Implement a brand agent that serves identity and rights.
  </Card>
  <Card title="get_rights" icon="magnifying-glass" href="/dist/docs/3.2.0-rc.4/brand-protocol/tasks/get_rights">
    Search for licensable talent with pricing and availability.
  </Card>
  <Card title="acquire_rights" icon="file-signature" href="/dist/docs/3.2.0-rc.4/brand-protocol/tasks/acquire_rights">
    Submit a binding request and receive generation credentials.
  </Card>
  <Card title="update_rights" icon="arrows-rotate" href="/dist/docs/3.2.0-rc.4/brand-protocol/tasks/update_rights">
    Modify an existing rights grant — extend, pause, adjust.
  </Card>
  <Card title="Certification" icon="graduation-cap" href="/dist/docs/3.2.0-rc.4/learning/tracks/buyer">
