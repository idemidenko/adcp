...
training seller supports the exact `3.2-rc.3` wire bundle with
`@adcp/sdk@14.0.0-rc.40`. Verify those advertised capabilities before the lab;
...
```json
{
  "$schema": "https://adcontextprotocol.org/schemas/3.2.0-rc.3/media-buy/request-proposals-request.json",
...
- Read `get_adcp_capabilities.adcp.supported_versions` and `media_buy.lifecycle_tools`, pin `adcp_version: "3.2-rc.3"` for this lab, and validate the echoed served release before using targeting-aware discovery.
